<section class="section maxw1035 margin-bottom-4-em">
    <br>
    <h2 class="centered">HOW IT WORKS</h2>
    <div id="how-it-works" class="three-col-grid">
        <div>
            <img id="img-shopping" class="image-how-it-works" src="../assets/images/shopping.png" alt="Shopping" />
            <h2 class="centered">Shop items</h2>
            <p class="centered">Choose the African foods you want, then select "Subscribe & save" on the
                products details page.</p>
        </div>
        <div>
            <img id="img-reminder" class="image-how-it-works" src="../assets/images/reminder.svg" alt="Reminder" />
            <h2 class="centered">Create subscription</h2>
            <p class="centered">Select the quantity and schedule. No fees. Cancel anytime.</p>
        </div>
        <div>
            <img id="img-gifts" class="image-how-it-works" src="../assets/images/gifts.svg" alt="Gifts" />
            <h2 class="centered">Claim your gifts</h2>
            <p class="centered">Select the quantity and schedule. No fees. Cancel anytime.</p>
        </div>

    </div>
</section>
